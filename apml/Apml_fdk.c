...
/* Processor-wide read Hardware Thermal Control register */
uint32 apml_read_htc_reg(
	APML_DEV_CTL	*ctl,
	uint8		proc,
	uint32		*reg_val_h,
	uint32		*reg_val_l,
	int			BMCInst)
{
        uint8		proc_addr;
	uint32		res, reg_id;
	const CHAR	*cmd_id = "RMI READ HTC";
	...
	res = apml_read_rmi_msr(ctl, proc_addr, ctl->rmi_core_target,
		cmd_id, reg_id, reg_val_h, reg_val_l,BMCInst);
...
/* Processor-wide write Hardware Thermal Control register */
uint32 apml_write_htc_reg(
	APML_DEV_CTL	*ctl,
	uint8		proc,
	uint8		reg_val_size,
	uint32		reg_val_h,
	...
	int			BMCInst)
{
        uint8		proc_addr, proc_ndx;
        uint32		res, reg_id;
        const CHAR	*cmd_id = "RMI WRITE HTC";

		reg_id = 0xC001003E;

	if ((reg_val_size == 0) || (reg_val_size > 8))
		return(APML_XS_DATA_LEN);
	...
	res = apml_write_rmi_msr(ctl, proc_ndx, proc_addr, ctl->rmi_core_target,
		cmd_id, reg_id, reg_val_size, reg_val_h, reg_val_l,BMCInst);
...
/* Core-specific read Current P-state */
uint32 apml_read_pstate(
	APML_DEV_CTL	*ctl,
	uint8		proc,
	uint8		core,
	uint8		*pstate,
	int			BMCInst)
{
        ...
        const CHAR	*cmd_id = "RMI READ PSTATE";
	...
	*pstate = 0;
	...
	res = apml_read_rmi_msr(ctl, proc_addr, core_ndx, cmd_id, reg_id, 
		&reg_val_h, &reg_val_l,BMCInst);

	if (res == APML_SUCCESS)
		*pstate = (reg_val_l & 0xFF);
...
/* Processor-wide read P-state Limit
 *
 * limit_active will be APML_TRUE if a P-state Limit is in effect, otherwise
 * it will be APML_FALSE. */
uint32 apml_read_pstate_limit(
	APML_DEV_CTL	*ctl,
	uint8		proc,
	uint8		*limit_active,
	uint8		*pstate_limit,
	int			BMCInst)
{
        uint8		proc_addr;
	uint32		res, reg_id, reg_val_h, reg_val_l;
        const CHAR	*cmd_id = "RMI READ PSTATE LIMIT";


	reg_id = 0xC0010072;

	*limit_active = APML_FALSE;
	*pstate_limit = 0;
...
//	proc_ndx = proc;
	...
	if (res == APML_SUCCESS) {
		if ((reg_val_l & 1) == 1)
			*limit_active = APML_TRUE;
		else
			*limit_active = APML_FALSE;

		*pstate_limit = (reg_val_l >> 8) & 0xFF;
	}
...
/* Processor-wide write P-state Limit */
uint32 apml_write_pstate_limit(
	APML_DEV_CTL	*ctl,
	uint8		proc,
	uint8		limit_active,
	uint8		pstate_limit,
	int			BMCInst)
{
        ...
        const CHAR	*cmd_id = "RMI WRITE PSTATE LIMIT";
	...
	reg_val_l = pstate_limit * 256;
	if (limit_active == APML_TRUE)
		reg_val_l++;

	res = apml_write_rmi_msr(ctl, proc_ndx, proc_addr, 
		ctl->rmi_core_target, cmd_id, reg_id, 2, 0, reg_val_l,BMCInst);
...
/* Processor-wide read Maximum P-state */
uint32 apml_read_max_pstate(
	APML_DEV_CTL	*ctl,
	uint8	proc,
	uint8	*val,
	int		BMCInst)
{
	uint8		proc_addr;
	uint32		res, reg_id, reg_val_h, reg_val_l;
	const CHAR	*cmd_id = "RMI READ MAX PSTATE";
	...
	int			BMCInst)
{
	...
	*data = 0;
	...
	res = user_get_proc_addr(dev, proc, option, &proc_addr);
	if (res != APML_SUCCESS)
		return(res);
	...
	int			BMCInst)
{
	...
	*data = 0;
	...
	res = user_get_proc_addr(dev, proc, PROC_USE_TSI, &proc_addr);
	if (res != APML_SUCCESS)
		return(res);
	...
	int			BMCInst)
{
	...
	msg_rd_bytes++;
	addr = proc_addr * 2; /* Convert from 7-bit to 8-bit SMBus address */

	msg_wr_bytes = wr_len;

	/* APML adds a PEC before the repeated start to ensure processor 
 	 * data is not corrupted by bus errors. */
	crc = apml_pec(0, 1, &addr);
	crc = apml_pec(crc, msg_wr_bytes, wr_data);
	wr_data[msg_wr_bytes] = crc;
	msg_wr_bytes++;
