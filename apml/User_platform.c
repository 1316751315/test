...
uint32 user_platform_init(APML_DEV_CTL *dev,int BMCInst)
{
	...
	for (i = 0; i < APML_MAX_PROCS; i++) {
		...
		res = apml_read_tsi_reg(dev, i, 1, &tmp8,BMCInst);		
	...
	int			BMCInst)
{
	...
	if(1)
    {
        for (retries = 0; retries < MAX_I2C_RETRIES; ++retries)
        {
            ...
            uerr = apml_smbus_write_data(get_i2c_bus_name(dev->bus_id), proc_addr, (uint8*) &reg, wr_len);
            if(uerr >= 0){
                break;
            }
        }
	...
	int			BMCInst)
{
	...
	if(1)
    {
        for (retries = 0; retries < MAX_I2C_RETRIES; ++retries)
        {
            ...
            uerr = apml_smbus_write_data(get_i2c_bus_name(dev->bus_id), proc_addr, (uint8*) &writeBuf[0], wr_len);
            if(uerr >= 0){
                break;
            }
        }
	...
	int			BMCInst)
{
	...
	if(1)
    {
        for (retries = 0; retries < MAX_I2C_RETRIES; ++retries)
        {
				 ...
				 uerr = apml_smbus_read_byte(get_i2c_bus_name(dev->bus_id),proc_addr, (uint8*) &reg, (uint8 *) data, wr_len, rd_len);
                if(uerr >= 0){
                    break;
                }
        }
	...
	int			BMCInst)
{
	...
	if(1)
    {
        for (retries = 0; retries < MAX_I2C_RETRIES; ++retries)
        {
             ...
             uerr = apml_smbus_read_byte(get_i2c_bus_name(dev->bus_id),proc_addr, (uint8*) &reg, (uint8 *) data, sizeof(uint8), sizeof(uint16));
            if(uerr >= 0){
                break;
            }
	...
	int			BMCInst)
{
	...
	if(1)
    {
        for (retries = 0; retries < MAX_I2C_RETRIES; ++retries)
        {
            ...
            uerr = apml_smbus_read_byte(get_i2c_bus_name(dev->bus_id),proc_addr, xmt_data, rcv_data, xmt_len, rcv_len);
            if(uerr >= 0){
                break;
            }
        }
